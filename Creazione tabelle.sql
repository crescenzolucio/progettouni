...
BEGIN
	...
	IF(((DATE_PART('day', CAST(NEW.FINE_PROIEZIONE AS TIME) - CAST(NEW.INIZIO_PROIEZIONE AS TIME)) * 24 + 
               DATE_PART('hour', CAST(NEW.FINE_PROIEZIONE AS TIME) - CAST(NEW.INIZIO_PROIEZIONE AS TIME))) * 60 +
               DATE_PART('minute', CAST(NEW.FINE_PROIEZIONE AS TIME) - CAST(NEW.INIZIO_PROIEZIONE AS TIME))) < MINUTI_FILM) THEN
		RAISE EXCEPTION 'LA DURATA DEL FILM É MINORE RISPETTO ALLA DURATA DELLA PROIEZIONE';
	END IF;
	RETURN NEW;
END; 
$$ language plpgsql;
